...
This skill is project-scoped and lives inside the repository at `.claude/skills/project-blog/`. It is not a global or user-level skill and is not meant for Claude Code on the web.
...
Use `references/svg-cover-template.svg` as the base — a 1280×720 dark-gradient banner with a two-line title, an accent bar, a subtitle, and a macOS-style terminal window showing 2–4 command lines. Fill in:

- the two title lines;
- the accent-bar color (keep one consistent accent, e.g. orange);
- the subtitle;
- the terminal command lines (prompt lines in light blue, slash commands in amber).

Write the result to `src/content/posts/image/YYMMDD_01.svg` and set frontmatter `image: './image/YYMMDD_01.svg'`. Keep the same visual language across posts so covers stay consistent.
...
The search script needs an Unsplash Access Key. Keep it in a `.env` file at the skill root (`.claude/skills/project-blog/.env`), which is git-ignored:
